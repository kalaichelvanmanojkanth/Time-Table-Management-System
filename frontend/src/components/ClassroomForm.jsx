...
function ClassroomForm({ activeClassroom, onCancel, onSubmit, saving }) {
  ...
  const validateForm = (formData) => {
    ...
    if (!building) {
      ...
      newErrors.building = "Building must be less than 50 characters";
    }
    ...
    if (resources) {
      const resourceArray = resources.split(",").map((item) => item.trim());
      if (resourceArray.length > 10) {
        newErrors.resources = "Maximum 10 resources allowed";
      }
    ...
    <form className="panel form-panel" onSubmit={handleSubmit}>
      ...
      <label>
        ...
        <input
          ...
          placeholder="projector, computers, smart board"
        />
