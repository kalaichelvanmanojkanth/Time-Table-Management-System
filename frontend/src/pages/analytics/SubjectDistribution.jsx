...
import {
  FaArrowLeft, FaBookOpen, FaExclamationTriangle, FaInfoCircle, FaTimes, FaSync,
} from 'react-icons/fa';
...
/* ── Build subjects from real timetable entries ── */
function buildSubjectsFromEntries(entries, setup) {
  const { subjectStats } = buildAnalyticsFromEntries(entries, setup || {});
  const maxSlots = Math.max(...subjectStats.map(s => s.slots), 1);

  return subjectStats.map((s, idx) => {
    // Find the teacher most frequently associated with this subject
    const teacherCount = {};
    entries
      .filter(e => e.subject === s.name)
      .forEach(e => { teacherCount[e.teacher] = (teacherCount[e.teacher] || 0) + 1; });
    const teacher = Object.entries(teacherCount).sort((a, b) => b[1] - a[1])[0]?.[0] || '';
    ...
    return {
      ...
      dept:        'General',
      teacher,
      hoursPerWeek: s.slots,
      maxHours:    Math.ceil(maxSlots * 1.2), // 20% over max triggers "over" status
      classes:     [],
    };
  });
}

/* ── Validate & add subject modal ── */
...
function AddSubjectModal({ onClose, onAdd, existing, availableTeachers }) {
  const [form, setForm]     = useState({ name: '', dept: 'General', teacher: '', hoursPerWeek: '', maxHours: '' });
  ...
  const DEPTS = ['General', 'Computer Science', 'Mathematics', 'Physics', 'Electronics'];
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <select value={form.teacher} onChange={e => set('teacher', e.target.value)} className="w-full text-sm px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-navy dark:text-slate-100 focus:outline-none focus:border-primary transition-colors">
              ...
              {availableTeachers.map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
        </div>
...
export default function SubjectDistribution() {
  ...
  const [extraSubs,  setExtraSubs]  = useState([]);   // user-added subjects
  const [tick,       setTick]       = useState(0);

  /* ── Load real data ── */
  const { baseSubjects, hasRealData, availableTeachers } = useMemo(() => {
    const entries = loadEntries();
    const setup   = loadSetup();

    if (!entries || entries.length === 0) {
      return {
        baseSubjects:      DEMO_SUBJECTS,
        hasRealData:       false,
        availableTeachers: ['Dr. Anita Sharma', 'Prof. Ravi Kumar', 'Dr. Preethi Nair', 'Mr. Suresh Babu', 'Dr. Meera Krishnan', 'Prof. Karthik Rajan'],
      };
    }

    const generatedSubs = buildSubjectsFromEntries(entries, setup);
    const teachers      = [...new Set(entries.map(e => e.teacher).filter(Boolean))];

    return { baseSubjects: generatedSubs, hasRealData: true, availableTeachers: teachers };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tick]);

  /* Merge base + user-added */
  const subjects = useMemo(() => {
    const baseKeys = new Set(baseSubjects.map(s => s.name.toLowerCase()));
    const extras   = extraSubs.filter(s => !baseKeys.has(s.name.toLowerCase()));
    return [...baseSubjects, ...extras];
  }, [baseSubjects, extraSubs]);
    ...
    <div className="min-h-screen bg-surface dark:bg-navy font-sans text-navy dark:text-slate-100 antialiased">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-5 sm:px-8 py-4 flex items-center gap-3 sticky top-0 z-40 shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            id="refresh-subject-distribution"
          ...
          <button id="add-subject-btn" onClick={() => setShowModal(true)} className="text-xs font-bold text-white bg-gradient-to-r from-secondary to-violet-700 px-4 py-2 rounded-xl shadow-md hover:from-indigo-700 transition-all">
            + Add Subject
          </button>
        </div>
      </header>
      ...
      {!hasRealData && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-800 px-5 sm:px-8 py-3 flex items-center gap-3">
          ...
          <span className="text-sm text-amber-700 dark:text-amber-300">
            ...
            and save your setup to see real subject analytics.
          </span>
        </div>
      )}
        ...
        <Reveal>
          <section className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xl p-6 sm:p-8">
            ...
            {noData || classTotals.length === 0
              ? <div className="h-28 flex items-center justify-center text-muted dark:text-slate-500 text-sm">No class data — assign subjects to classes or generate timetable</div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {['all', 'over', 'under', 'unscheduled', 'no-teacher', 'ok'].map(k => (
                  <button key={k} id={`subj-filter-${k}`} onClick={() => setFilter(k)}
                      ...
                      <div key={s.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-md p-5 hover:shadow-lg transition-shadow">
                        ...
                        {(s.classes || []).length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mt-3">
                            {s.classes.map(c => <span key={c} className="text-[10px] font-semibold bg-slate-100 dark:bg-slate-700 text-muted dark:text-slate-300 rounded-full px-2.5 py-0.5">{c}</span>)}
                          </div>
                        )}
